...
Każdy z tych elementów wykonawczych nawiązuje komunikację z Raspberry Pi poprzez podłączenie do odpowiednich pinów GPIO mikrokontrolera. Zastosowano również dodatkowe rezystory w połączeniu szeregowym z diodą LED w celu ograniczenia przepływu prądu. 
\newpage
...
\section{Projektowanie układu działania}
 Płytka prototypowa umożliwia połączenie elementów elektronicznych do określonych pinów GPIO na płycie mikrokontrolera. Poprzez wpisanie określonych komend w aplikacji, możliwe będzie wysyłanie poleceń do Raspberry Pi. Będą one odpowiedzialne za wykonywanie operacji na podłączonych urządzeniach wykonawczych. W celu sprawdzenia funkcjonalności komunikacji tekstowej zaprojektowano stanowisko układu, w którego skład wchodzą elementy takie jak: 
\begin{itemize}  
	\item \textbf{Dioda LED koloru czerwonego},
	\\
\item \textbf{Dioda LED koloru zielonego}, 
	\\
\item \textbf{Dioda LED koloru żółtego}, 
	\\
	\item \textbf{Serwomechanizm}
	\\
\end{itemize}
\begin{figure}[h]
...
\caption{Układ połączenia z zastosowaniem mikrokontrolera}
\label{fig49}
\end{figure}
...
W celu stworzenia archiwum danych zaimplementowano klasę, która pomoże zapisać wiadomości tekstowe w aplikacji. Domyślnie, obiekt posiada zdefiniowaną ścieżkę do pliku JSON przechowującego dane archiwum. W trakcie pierwszego uruchomienia aplikacji, jeżeli plik nie istnieje, zostanie on automatycznie utworzony. W innym przypadku, zostanie wczytana jego zawartość oraz załadowana do pamięci aplikacji.
